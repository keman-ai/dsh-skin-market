...
function githubSpecOf(repoUrl) {
	...
	return match === null ? void 0 : `github:${match[1]}/${match[2]}`;
}
...
function safeSpec(spec) {
	...
	return bare.startsWith("@deepseek-ai/") || bare.startsWith("dsh-base") ? void 0 : spec;
}
/**
* 从集市给的整条安装命令里取出 spec。
...
function specFromCommand(command) {
	...
	if (at < 0 || at + 1 >= words.length) return void 0;
	const spec = words[at + 1];
	return spec === void 0 || spec.startsWith("-") || spec.startsWith("<") ? void 0 : spec;
}
/**
* 上游一行 → 目录条目。
...
function normalizeEntry(raw) {
	...
	const installSpec = safeSpec(str(row.installSpec) ?? specFromCommand(str(row.installCommand)) ?? str(row.packageName) ?? githubSpecOf(repoUrl));
	...
	return {
		...
		...installSpec !== void 0 ? { installSpec } : {},
...
function run(command, args, cwd, emit) {
	return new Promise((resolve$1) => {
		...
		const timer = setTimeout(() => {
			if (settled) return;
			child.kill("SIGKILL");
			output += `\n[超时] 命令超过 ${PNPM_TIMEOUT_MS / 1e3} 秒未结束，已终止。`;
		}, PNPM_TIMEOUT_MS);
		const consume = (chunk) => {
