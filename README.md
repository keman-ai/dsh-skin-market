...
| **浏览器** | 装皮肤要求**本机直连**（loopback） | 远程访问时能逛能搜，安装被拒（见安全边界）|
...
| 面 | 处置 |
|---|---|
| 谁能装 | 只有**本机直连**的浏览器。带 `x-forwarded-*` 的请求一律拒绝 —— 反代转进来的请求 socket 看着也是 127.0.0.1，只认地址会把这条保证架空 |
| 装什么 | 只装 dsh.a2hmarket.ai 收录过的 spec，手输任意包名不放行；`@deepseek-ai/*` 一律判为不可装，防止一条填错的元数据把宿主自己的包塞进你的 profile |
| 构建脚本 | 保持 pnpm 默认拒绝。需要时弹确认讲清楚「这等于允许该包代码在你机器上执行，且不在 agent 沙箱内」，同意后才写 `allowBuilds` 并重试 —— 不代写、不静默 |
| 装失败 | 事务化：`pnpm add` 成功了才动配置；挂载失败就把包卸回去，不留半装状态 |
| 改配置 | 内联进来的每一条都挂 `# skin-market:<包名>` 归属注释，卸载只摘认得的那些。你自己写的行和注释原样保留；patch 文件不是条目数组时直接报错，绝不覆盖 |
| 改皮肤的行 | 只在一种情况下动：作者把「挂载自己」误写成「改一个不存在的行」（那行本来就不生效）。此时补上 `insert` 并把行 id 换成 `skin:<包名>`，原 id 记在注释里。作者写对的行连 id 都不碰 —— 同包多行可能靠 id 互相引用 |
